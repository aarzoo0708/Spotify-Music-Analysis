...
def load_and_clean_data():
    ...
    # Convert duration from milliseconds to minutes for easier readability
...
# Apply Filters using standard Pandas operations
filtered_df = df.copy()
...
if search_term:
    ...
    mask_artist = filtered_df["artists"].str.contains(search_term, case=False, na=False)
    mask_track = filtered_df["track_name"].str.contains(search_term, case=False, na=False)
    filtered_df = filtered_df[mask_artist | mask_track]
...
with tab3:
    ...
    if "artists" not in filtered_df.columns:
        ...
        artist_list = sorted(filtered_df["artists"].dropna().unique().tolist())
        ...
        if selected_artist:
            ...
            artist_top = artist_data.sort_values(by=sort_col, ascending=False).head(10)
...
with tab4:
    ...
    if available_features:
        ...
        with feat_col2:
            ...
            ax.scatter(filtered_df[x_feat], filtered_df[y_feat], alpha=0.3, color="#F2C94C")
    ...
    try:
        ...
        if not mood_df.empty:
            ...
            if "popularity" in mood_df.columns:
                mood_df = mood_df.sort_values(by="popularity", ascending=False)
...
with tab6:
    ...
    with p_col1:
        ...
        pl_genres = ["Any"]
        if "track_genre" in filtered_df.columns:
            pl_genres += sorted(filtered_df["track_genre"].dropna().unique().tolist())
    ...
    if st.button("Generate Playlist", type="primary"):
        # 1. Start with the globally filtered data
        playlist = filtered_df.copy()
        
        # 2. Apply Popularity Filter
        if "popularity" in playlist.columns:
            playlist = playlist[playlist["popularity"] >= pl_min_pop]
            
        # 3. Apply Genre Filter
        if pl_target_genre != "Any" and "track_genre" in playlist.columns:
            playlist = playlist[playlist["track_genre"] == pl_target_genre]
            
        # 4. Apply Energy Filter
        if "energy" in playlist.columns:
            if pl_energy == "High Energy (>0.7)":
                playlist = playlist[playlist["energy"] > 0.7]
            elif pl_energy == "Low Energy (<0.4)":
                playlist = playlist[playlist["energy"] < 0.4]
        ...
        if playlist.empty:
            ...
            if "popularity" in final_playlist.columns:
                final_playlist = final_playlist.sort_values(by="popularity", ascending=False)
